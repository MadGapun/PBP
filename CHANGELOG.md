...
> v1.7.23 in die Beta-Linie. Keine Schema-Migration (v52 unveraendert).

### Fixed
Inhaltlich identisch zu **v1.7.22** und **v1.7.23** — Details stehen in
den beiden Eintraegen weiter unten. In Kurzform:

- Der Anzeigentext wird vollstaendig gespeichert statt bei 2000 Zeichen
  gekappt (#952). In der Beta-Linie zusaetzlich wichtig: der
  unveraenderliche Snapshot (#687) bekommt jetzt den vollen Text —
  vorher wurde der halbe Text auf Dauer festgeschrieben.
- MUSS-Begriffe sind wieder ein echtes Tor, Fachscore und Rahmenscore
  sind getrennt (#940, #942).
- Erkannte Wiedergaenger werden aussortiert statt nur markiert, mit
  eingeklappter Rueckhol-Liste (#941).
- Nachfassungen sagen, was zu tun ist; ueberholte werden hinfaellig
  (#945).
- Statistik und Bewerbungsbericht rechnen und klassifizieren richtig
  (#943, #944).
- Elwosa wiederholt sich nicht mehr (#924), der Quellen-Check prueft
  Inhalt statt nur den HTTP-Status (#808), Abschluss-Anforderungen
  werden zuverlaessiger erkannt (#955).
- Ausgehende Fehlerberichte werden gegen den eigenen Bestand geprueft
  (#946).
...
laufen auf derselben Plattform.

### Fixed

- **Jobware liefert jede Stelle einer Ergebnisseite** (#1041). Bis hierher
  kamen je Suchlauf 1-2 Stellen an: die Auswahl traf die Bestandteile der
  Karten statt der Karten, und der Kartenweg lief nur fuer den ersten
  Suchbegriff. Nachgemessen an einer echten Ergebnisseite: 20 statt 1.
- **Jobware-Stellen tragen den richtigen Titel, die Firma und einen
  geocodierbaren Ort** (#1041). Der Titel kam aus dem Knopf "Job ansehen"
  (`Job"<Titel>"ansehen`), die Firma blieb "Unbekannt", und der Ort stand
  mit einem versteckten "in" zusammengeschrieben da (`inBerlin`) — ohne
  Koordinaten, also ohne Entfernung. Bereits gespeicherte Stellen werden
  beim naechsten Fund auf ihrer eigenen Zeile korrigiert, statt als
  Duplikat neben dem kaputten Eintrag zu landen.
- **ingenieur.de fuehrt jede Stelle einmal, mit Firma und Ort** (#1042).
  Jede Stelle stand 2-4-mal in der Liste, fuer jede Kopie wurde die
  Detailseite erneut geholt, und die Firma war ein HTML-Kommentar.
  Nachgemessen: 15 Stellen statt 3, jede mit Firma und Ort.

### Changed

- **Ein Kartenleser fuer beide Boersen**
  (`job_scraper/jobboerse_karten.py`) statt zwei Fassungen desselben
  Fehlers.

### Bekannt

- Die Suche ist bei beiden Boersen weiter bundesweit. Jobware wertet den
  Ort laut Bericht aus; ihn zu nutzen aendert die Treffermenge und ist ein
  eigener Schritt.
...
1. **ZIP herunterladen:** [PBP-1.7.104.zip](https://github.com/MadGapun/PBP/archive/refs/tags/v1.7.104.zip)
2. **Entpacken:** Rechtsklick auf die ZIP → *„Alle extrahieren..."* → Zielordner wählen (z.B. `C:\PBP`). Darin liegt ein Unterordner `PBP-...` — dort hinein wechseln.
3. **Installieren:** Doppelklick auf **`INSTALLIEREN.bat`**
4. Das Setup lädt Python, alle Pakete und Chromium herunter (~3–5 Minuten) und konfiguriert Claude Desktop.
5. Auf dem Desktop liegt jetzt eine Verknüpfung **„PBP Bewerbungs-Portal"** — Doppelklick startet das Dashboard.
...
## [1.7.103] - 2026-09-14 — Karte und Popup sagen dasselbe

Ein Fehler aus #1044 in der Stellenliste.

### Fixed

- **Das Popup "Stellendetails" zeigt eine Stelle wie die Karte** (#1044).
  Es war eigener Code und las die Rohwerte: keine Entfernung, die
  Anstellungsform klein und grau ("festanstellung"), kein Umfang, ein
  Gehalt mit Bindestrich, dem Rohwert "(jaehrlich)" und "(geschaetzt)",
  und "Unbekannt" statt "Unbekannte Firma". Jetzt stehen Entfernung,
  Anstellungsform in Text und Farbe, Umfang, Gehalt und der Platzhalter
  fuer eine fehlende Firma im Popup genau wie auf der Karte.

### Changed

- **Karte und Popup lesen eine gemeinsame Fassung**
  (`frontend/src/lib/stellenAngaben.js`), mit eigenem Test im CI. Damit
  laufen die beiden Ansichten nicht wieder auseinander.
...
1. **ZIP herunterladen:** [PBP-1.7.103.zip](https://github.com/MadGapun/PBP/archive/refs/tags/v1.7.103.zip)
2. **Entpacken:** Rechtsklick auf die ZIP → *„Alle extrahieren..."* → Zielordner wählen (z.B. `C:\PBP`). Darin liegt ein Unterordner `PBP-...` — dort hinein wechseln.
3. **Installieren:** Doppelklick auf **`INSTALLIEREN.bat`**
4. Das Setup lädt Python, alle Pakete und Chromium herunter (~3–5 Minuten) und konfiguriert Claude Desktop.
5. Auf dem Desktop liegt jetzt eine Verknüpfung **„PBP Bewerbungs-Portal"** — Doppelklick startet das Dashboard.
...
was die Rueckfrage verspricht.

### Fixed

- **Ohne Routing-Schluessel rechnet PBP wieder mit der Luftlinie** (#1037).
  Die Rueckfrage beim Entfernen sagte "PBP rechnet danach wieder mit der
  Luftlinie" — an Stellen, an denen schon eine Fahrstrecke gespeichert
  war, rechneten Score, Scoring-Regler und Auto-Aussortierung aber weiter
  mit ihr. Jetzt zaehlt eine gespeicherte Fahrstrecke nur, solange ein
  Schluessel eingerichtet ist; angezeigt bleibt sie. Die bereits
  gespeicherten Scores zieht `scores_neu_berechnen()` nach, und das sagt
  die Rueckfrage jetzt auch.

### Changed

- **Der Entfernungs-Zuschlag der Scoring-Regler nutzt dieselbe Grenze wie
  Score und Automatik.** Er hielt eine eigene, vierte Vorgabetabelle, die
  v1.7.99 (#1036) uebersehen hatte. Die Zahlen waren gleich; es gibt jetzt
  nur noch eine Stelle, an der sie stehen.
...
1. **ZIP herunterladen:** [PBP-1.7.100.zip](https://github.com/MadGapun/PBP/archive/refs/tags/v1.7.100.zip)
2. **Entpacken:** Rechtsklick auf die ZIP → *„Alle extrahieren..."* → Zielordner wählen (z.B. `C:\PBP`). Darin liegt ein Unterordner `PBP-...` — dort hinein wechseln.
3. **Installieren:** Doppelklick auf **`INSTALLIEREN.bat`**
4. Das Setup lädt Python, alle Pakete und Chromium herunter (~3–5 Minuten) und konfiguriert Claude Desktop.
5. Auf dem Desktop liegt jetzt eine Verknüpfung **„PBP Bewerbungs-Portal"** — Doppelklick startet das Dashboard.
...
hat den neuen Extraktor am eigenen Bestand nachgezogen — 823 geprüft,
122 geändert, 6 geleert, **113 belegte Angaben statt vorher 11** — und
dabei drei Lücken gefunden.

### Fixed

- **Die Plausibilitätsgrenze prüfte nur den ersten Wert.** Bei einer
  Spanne kam der zweite ungeprüft durch: aus einer Telefonnummer mit
  Durchwahl wurde ein Jahresgehalt von 10 EUR, und zwar als **belegt**
  gespeichert. Die Prüfung stand vor der Sortierung, und die Sortierung
  hat den geprüften Wert vertauscht. Jetzt müssen beide Werte die
  Grenze passieren, sonst wird die ganze Fundstelle verworfen — lieber
  gar kein Wert als ein falscher.
- **„Eur" wurde ohne Wortgrenze gesucht** und qualifizierte damit jede
  Zahl neben „Europastr.", „Eurotunnel" oder „europaweit" als Betrag.
  Dritter Fall dieser Klasse nach „ki" in „Kita" (#970) und „us" in
  „Kundenservice" (#996).
- **Der Punkt als Dezimaltrenner** (nicht gemeldet, beim Nachmessen
  gefunden): `72.5-103k EUR` wurde als „5 bis 103" gelesen, scheiterte
  an der Grenze, und der Einzelwert-Pfad machte daraus 103.000 bis
  113.300 — also die **Obergrenze als Untergrenze**. Drei Anzeigen sahen
  dadurch besser bezahlt aus als sie sind, und genau dieser Wert geht in
  den Score ein.

### Added

- **`gehalt_setzen`** — eine Gehaltsangabe von Hand setzen oder löschen,
  analog zu `dokument_text_setzen`. Bis hierher ließ sich ein falsch
  erkanntes Gehalt gar nicht korrigieren: `gehalt_extrahieren` liest
  denselben Text wieder gleich, und `stelle_bearbeiten` kennt die
  Gehaltsfelder nicht. **Damit war jeder Fehltreffer dauerhaft**, nicht
  nur der gemeldete. Ein von Hand gesetzter Wert überlebt jeden
  automatischen Lauf und jeden erneuten Suchlauf. MCP-Tools 231.

### Gemessen

Über 1.337 Anzeigen mit Text, alter gegen neuen Extraktor:
...
1. **ZIP herunterladen:** [PBP-1.7.82.zip](https://github.com/MadGapun/PBP/archive/refs/tags/v1.7.82.zip)
2. **Entpacken:** Rechtsklick auf die ZIP → *„Alle extrahieren..."* → Zielordner wählen (z.B. `C:\PBP`). Darin liegt ein Unterordner `PBP-...` — dort hinein wechseln.
3. **Installieren:** Doppelklick auf **`INSTALLIEREN.bat`**
4. Das Setup lädt Python, alle Pakete und Chromium herunter (~3–5 Minuten) und konfiguriert Claude Desktop.
5. Auf dem Desktop liegt jetzt eine Verknüpfung **„PBP Bewerbungs-Portal"** — Doppelklick startet das Dashboard.
...
> v1.7.21 in die Beta-Linie. Keine Schema-Migration (v52 unveraendert).

### Fixed
- **Zwei tote Quellen wiederhergestellt (#925, #926)**: Eine
  Personaldienstleister-Seite stand als aufgegeben markiert, lieferte
  aber weiterhin 25 Stellen pro Abruf — der Parser suchte die Daten an
  der falschen Stelle im Seitenquelltext. Eine Projektboerse lieferte
  null Treffer, weil die hinterlegten Ausweich-Adressen unvollstaendig
  waren; mit der Korrektur sind es 87 Projekte. Beide liefern jetzt
  Direktlinks und echte Verguetungsangaben.
- **Dritte Quelle repariert, fuenf ausgegraut (v1.7.20)**: Eine
  Ingenieur-Jobboerse lief auf einem Pfad, der mit 404 antwortet.
  Fuenf weitere Quellen sind nachweislich tot und werden jetzt sichtbar
  als defekt gefuehrt — mit Begruendung und Pruefdatum, statt still
  mitgeschleppt zu werden.
- **Keine Sackgassen im leeren Zustand (#927)**: Gemessen ueber alle 53
  argumentlos aufrufbaren Funktionen auf einer frischen Datenbank — 18
  endeten in einer Sackgasse, jetzt sind es 6. Details im Eintrag zu
  v1.7.21.

### Added
- `services/nutzerfuehrung.py` — zentrale Wegweiser-Texte.
- `job_scraper/hydration.py` — liest Stellendaten aus dem
  Seitenquelltext moderner Karriereseiten; auf weitere Portale
  uebertragbar.
...
## [1.8.0-beta.13] - 2026-08-18 — Nachzug: Phantom-Termine und Abschluss-Erkennung (#922, #918)

> **Prerelease.** Identischer Inhalt wie **v1.7.18** (Stable): zwei
> Datenqualitaets-Fehler, bei denen PBP etwas berichtete, das nicht in
> der Quelle steht. Schema **v52** unveraendert.

### Fixed
- **Phantom-Termine aus zitierten Mail-Threads (#922)**: der Import
  EINER Mail mit Antwortverlauf legte VIER Termine an — die Sendezeiten
  der zitierten Vorgaengermails, alle als "interview" mit erfundenen 60
  Minuten; `firma_kontext` meldete daraufhin fuenf Interviews statt
  einem. Zitierte Bereiche werden vor der Terminsuche abgeschnitten, ein
  Datum allein genuegt nicht mehr (ICS, Konferenzlink oder
  Terminvokabular noetig), aus Text abgeleitete Termine bekommen kein
  pauschales "interview". NEU `phantom_termine_bereinigen` fuer den
  Altbestand (Report mit Begruendung, Loeschen nur auf Anweisung).
- **Hochschulabschluss-Erkennung in beide Richtungen falsch (#918
  Defekt 2)**: sie lief ueber den gesamten Datensatz — eine
  Bewerberstatistik in den Notizen loeste den ATS-Risikohinweis bei
  einer Anzeige ohne Abschluss-Anforderung aus, waehrend "Educational
  Background: Bachelor's degree" gar nicht erkannt wurde. Notizen und
  Statistik-Zeilen raus, englische Muster und Oeffnungsklauseln
  ergaenzt, Whitespace-Glaettung.

### Changed
- **MCP-Tools: 216** (+`phantom_termine_bereinigen`)
