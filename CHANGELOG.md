...
- **Das MUSS-Tor ist umschaltbar (#968).** Trifft keiner deiner
  Pflichtbegriffe, wird eine Anzeige bisher verworfen — sie wird gar
  nicht erst gespeichert. Im dokumentierten Lauf aus #813 starben so
  **312 von 389 Rohtreffern**, bevor ein Mensch sie gesehen hat.

  Ob das richtig ist, hängt daran, **was deine Pflichtbegriffe
  nennen**:

  * **Techniken** („PLM", „SAP", „Python") — ihr Fehlen ist ein echter
    Beleg: die Anzeige gehört in ein anderes Fachgebiet. Dafür ist
    `hart` richtig.
  * **einen Beruf** („Pflegefachkraft", „Erzieherin") — ihr Fehlen sagt
    wenig, weil derselbe Beruf in vielen Anzeigen anders heißt. Dafür
    ist `gewichtet` richtig.

  Umstellen mit `muss_tor_setzen('gewichtet')`; der aktuelle Stand
  steht in `suchkriterien_anzeigen`.

- **In der Betriebsart `gewichtet`** wird die Stelle gespeichert und
  angezeigt, aber sie **kann nie über einer Stelle mit Pflichttreffer
  stehen**, sie trägt **höchstens die Hälfte dessen, was ein einzelner
  Pflichttreffer wert ist**, und sie steht in der Liste mit
  **„Kein Pflichttreffer"** an der Zeile. Je Suchlauf werden höchstens
  50 solcher Stellen behalten; die übrigen nennt der Filtertrichter.

### Changed

- **Die Rangfolge steht in der Sortierung, nicht im Score.** Eine
  Stelle mit Pflichttreffer darf abstürzen, wenn ein Malus sie
  herunterzieht — das ist die Entscheidung aus #942 und bleibt so. Sie
  darf dabei nur nicht unter eine Stelle rutschen, über deren Fach
  nichts bekannt ist. Dieselbe Bauform wie der Datengüte-Rang aus
  #989.

### Bewusst nicht

**Die Vorgabe bleibt `hart` — und das ist gemessen, nicht bequem.**

Gegen den eigenen Bestand (2.491 Anzeigen, Kopie im
Temp-Verzeichnis): 1.900 öffnen das Tor nicht, und **1.402 davon waren
bereits von Hand als fachfremd aussortiert worden**. Das Tor stimmt in
diesem Bestand also mit dem eigenen Urteil überein; die Vorgabe
umzustellen hätte rund 1.400 abgelehnte Anzeigen zurück in die Liste
gelegt.

Das Issue verlangt die Gewichtung ausdrücklich als Vorgabe. Diese
Messung spricht dagegen, deshalb ist beides gebaut und umschaltbar —
was fehlt, ist allein die Voreinstellung, und die ist eine
Entscheidung.

**Wer nichts umstellt, merkt von dieser Version nichts:** über alle
2.491 Anzeigen nachgerechnet ändert sich in der Vorgabe **kein
einziger Score**.

**Tests:** 3475 bestanden, 2 übersprungen (+26 neue).
...
1. **ZIP herunterladen:** [PBP-1.7.68.zip](https://github.com/MadGapun/PBP/archive/refs/tags/v1.7.68.zip)
...
F5 und Strg+F5 ändern nichts. Der Eindruck ist ein Aktualisierungsfehler.
Es war keiner — PBP hat gefiltert und es nicht gesagt.

Drei Befunde, acht Akzeptanzkriterien, dazu ein vierter, den ich beim
Nachsehen selbst gefunden habe.

### Fixed

- **Ein Filter, den du nie gesetzt hast, verbarg 7 von 8 Stellen (#1008
  Befund 1).** Der Score-Filter der Stellenliste bezog seinen
  Vorgabewert aus `min_score_schwelle`. Das ist laut PBPs **eigener
  Beschreibung** die Schwelle, ab der eine Stelle beim Suchlauf
  überhaupt *gespeichert* wird — ausdrücklich „wirkt während der Suche,
  nicht in der Liste". Der Anzeige-Filter heißt
  `schwellenwert/auto_ignore` und wirkt serverseitig; er stand bei dir
  auf 0, also aus. Die Liste startet jetzt ungefiltert.
- **Die Kennzahl-Karte widersprach sich selbst.** Sie trug die
  Überschrift „Aktive Stellen" über der Zahl der *angezeigten* und zwei
  Zeilen darunter die Notiz „8 aktiv gesamt, 7 durch Filter verborgen".
  Der Sidebar-Zähler war korrekt und sah dadurch falsch aus. Die
  Überschrift benennt jetzt die Größe, die darunter steht.
- **Über der Liste steht, was fehlt.** Sind Einträge durch Filter
  verborgen, nennt ein Hinweis ihre Zahl und die wirksamen Filter — mit
  einem Knopf „Filter aufheben" daneben.
- **Vier verschiedene Score-Werte für dieselbe Stelle (#1008 Befund 2).**
  Zwei Ursachen: `save_jobs` behält beim erneuten Speichern den höheren
  **alten** Gesamtwert, schrieb `fachscore`/`rahmenscore` aber
  bedingungslos neu — danach stand die Aufteilung des einen Laufs neben
  dem Gesamtwert eines anderen (genau dein „score 10.0 bei fachscore 0.0
  und rahmenscore 0.0"). Und `GET /api/jobs`, die Liste die du ansiehst,
  gab den **rohen** gespeicherten Wert aus, während MCP-Liste und
  Bericht den mit deinen Scoring-Reglern angepassten nannten. Beides
  läuft jetzt durch je ein Nadelöhr.
- **Wirkungslose Regler lagen wie echte Einstellungen im Bestand (#1008
  Befund 3).** `hochschulabschluss/fehlt` stand sogar in **jeder
  frischen Datenbank**, obwohl die Prüfung dahinter in v1.7.35 (#972)
  ersatzlos entfernt wurde; der Malus wurde an zwei Stellen in die
  Kriterien geschrieben und von keinem Rechenweg gelesen. Vorgabe
  entfernt, Altzeilen werden beim Start abgeräumt, die tote Kette ist
  weg. `pbp_diagnose` meldet verbliebene wirkungslose Regler.
- **Und derselbe Fehler im MCP, also im Kernstück von PBP.**
  `stellen_anzeigen` verwarf Stellen unter der Score-Schwelle still,
  zählte sie nur ins Log und antwortete dann *„Keine Stellen gefunden.
  Starte eine Jobsuche"* — während `pbp_diagnose` dieselben Stellen als
  aktiv meldete. Falsche Ursache, falscher nächster Schritt: eine Suche
  bringt nichts, wenn die Treffer längst da sind. Die Zahl steht jetzt
  in der Antwort, und die Meldung nennt den Weg zur Schwelle.

### Changed

- `fit_analyse` nennt neben dem Fachwert den Wert, der **in der Liste**
  steht, wenn beide auseinandergehen. Sie gleichzumachen war keine
  Option: der Fachwert wird gegen den erreichbaren Höchstwert gehalten,
  und in dem kommen die Regler nicht vor — eine Stelle, die alles
  trifft, muss weiterhin exakt 100 % ergeben (#999).
- Die Filter-Vorgabe der Stellenliste steht an **einer** Stelle, die
  alle drei Zurücksetz-Wege lesen.

### Removed

- `search_criteria` aus der Workspace-Antwort. **Das ist eine Korrektur
  an v1.7.50 (#993).** Dort wurde das Feld eingeführt, weil der
  Stellen-Tab `min_score_schwelle` als Filter-Vorgabe las; der falsche
  Zugriffspfad war damals der Fund, und die eigentliche Frage — ob diese
  Schwelle die Anzeige überhaupt filtern darf — blieb ungestellt. Der
  reparierte Zugriff hat einen seit beta.27 schlafenden Filter erst
  scharfgeschaltet. Mit dem Leser fällt der Lieferant.
- `get_hochschulabschluss_malus` samt ihren beiden Aufrufern.

**Tests:** 3380 bestanden, 2 übersprungen (+22 neue).
---
...
1. **ZIP herunterladen:** [PBP-1.7.62.zip](https://github.com/MadGapun/PBP/archive/refs/tags/v1.7.62.zip)
2. **Entpacken:** Rechtsklick auf die ZIP → *„Alle extrahieren..."* → Zielordner waehlen (z.B. `C:\PBP`). Darin liegt ein Unterordner `PBP-...` — dort hinein wechseln.
3. **Installieren:** Doppelklick auf **`INSTALLIEREN.bat`**
4. Das Setup laedt Python, alle Pakete und Chromium herunter (~3–5 Minuten) und konfiguriert Claude Desktop.
5. Auf dem Desktop liegt jetzt eine Verknuepfung **„PBP Bewerbungs-Portal"** — Doppelklick startet das Dashboard.
...
leeres, sechszeiliges Eingabefeld. Die gespeicherten Recherchen standen
darunter — und waren erst nach dem Scrollen zu sehen.

Wer nachsehen wollte, was er über eine Firma schon zusammengetragen hat,
sah als Erstes eine leere Fläche. Naheliegender Schluss: *ist wohl nicht
gespeichert*. Und dann schreibt man es eben nochmal.

### Fixed

- **Die gespeicherten Recherchen stehen jetzt oben, das Eingabefeld
  darunter.** Der Normalfall ist Nachsehen, nicht Schreiben — und die
  Reihenfolge auf dem Bildschirm ist eine Aussage darüber, was der
  Normalfall ist.

- **Das Eingabefeld ist zugeklappt** („Eigene Notiz hinzufügen"). Ein
  leeres Feld, das Platz einnimmt, sieht aus wie eine Lücke, die
  jemand füllen soll.

- **Der Knopf „Mit Claude aktualisieren" hat nicht aktualisiert.** Er
  kopierte `/firmen_recherche firma="…"` — **ohne die Bewerbungs-ID**,
  und ohne die speichert `firmen_recherche` seit v1.6 nichts. Der
  kopierte Prompt war also ein reiner Lesevorgang: Claude recherchierte,
  gab die Antwort im Chat aus, und in PBP kam nichts an. Wer sich darauf
  verlassen hat, hat seine Recherche verloren.

  Die ID hängt jetzt dran, und der Knopf heißt, was er tut:
  **„Prompt kopieren"**. Ein Knopf, der etwas anderes tut als sein Label
  sagt, ist teurer als ein fehlender Knopf — man verlässt sich darauf.

### Bekannte Grenze

Firmen-Recherchen liegen weiterhin an **zwei** Stellen (an der Stelle und
an der Bewerbung); dasselbe gilt für Notizen. Das zusammenzuführen ist
eine eigene Arbeit und steht als #956 und #957 offen — hier ändert sich
nur, was du siehst und was der Knopf kopiert.

Geschlossen: #958. Tests: 3234.
...
1. **ZIP herunterladen:** [PBP-1.7.57.zip](https://github.com/MadGapun/PBP/archive/refs/tags/v1.7.57.zip)
2. **Entpacken:** Rechtsklick auf die ZIP → *„Alle extrahieren..."* → Zielordner waehlen (z.B. `C:\PBP`). Darin liegt ein Unterordner `PBP-...` — dort hinein wechseln.
3. **Installieren:** Doppelklick auf **`INSTALLIEREN.bat`**
4. Das Setup laedt Python, alle Pakete und Chromium herunter (~3–5 Minuten) und konfiguriert Claude Desktop.
5. Auf dem Desktop liegt jetzt eine Verknuepfung **„PBP Bewerbungs-Portal"** — Doppelklick startet das Dashboard.
...
## [1.7.56] - 2026-09-09 — Bei welchem Score bewirbst du dich eigentlich?

Deine Frage vom 7. September: *„Wie hoch war der Score bei den Stellen,
auf die ich mich beworben habe, als Spanne und Durchschnitt? Vielleicht
kann man daraus lernen, auf welche Stellen ich mich bewerbe."*

Die Zahlen gab es — aber nur als Nebenprodukt der Schwellenkalibrierung,
nirgends als Kennzahl.

### Added

- **Die Score-Verteilung deiner Bewerbungen**, in der Statistik und im
  Dashboard: Spanne, Mittel, Median und die mittlere Hälfte, dazu eine
  Aufschlüsselung nach Ausgang (abgelehnt, Interview, Angebot …).

  Der Score ist eine **Einschätzung** der Passung — entschieden hast du.
  Die Verteilung zeigt deshalb etwas, das keine andere Zahl zeigt: wie
  weit dein Urteil vom Modell abweicht, und wo systematisch.

- **Bewerbungen unterhalb deiner Schwelle stehen einzeln da.** Das ist
  der interessanteste Teil: dort hast du dich bewusst gegen die
  Einschätzung entschieden. Häufen sie sich, sagt das mehr über die
  Kriterien als über die Stellen.

- **Im Dashboard mit Schwellenmarkierung** — ein Spannenbalken mit
  Median und deiner eingestellten Grenze, damit die Zahlen einen
  Bezugspunkt haben.

### Hinweis

Zwei Dinge werden **getrennt** ausgewiesen statt eingerechnet:

- **Score 0 heißt „kein Pflichtbegriff getroffen"** — die Stelle wurde
  nicht schlecht bewertet, sondern gar nicht beurteilt. Sie zählt nicht
  in Mittel und Median. Dass du dich trotzdem beworben hast, ist der
  eigentliche Befund: die Pflichtbegriffe decken diese Rollen nicht ab.
- **Bewerbungen ohne verknüpfte Stelle** haben keinen Score. Das ist
  eine Datenlücke, kein Nullwert — sie werden benannt, statt still zu
  verschwinden.

Die Kennzahl ist rein lesend und verändert keinen Score.
...
1. **ZIP herunterladen:** [PBP-1.7.56.zip](https://github.com/MadGapun/PBP/archive/refs/tags/v1.7.56.zip)
2. **Entpacken:** Rechtsklick auf die ZIP → *„Alle extrahieren..."* → Zielordner waehlen (z.B. `C:\PBP`). Darin liegt ein Unterordner `PBP-...` — dort hinein wechseln.
3. **Installieren:** Doppelklick auf **`INSTALLIEREN.bat`**
4. Das Setup laedt Python, alle Pakete und Chromium herunter (~3–5 Minuten) und konfiguriert Claude Desktop.
5. Auf dem Desktop liegt jetzt eine Verknuepfung **„PBP Bewerbungs-Portal"** — Doppelklick startet das Dashboard.
...
Fehlerberichte geraten, **fand sie nicht** — sobald der Name im Bericht
etwas kürzer stand als in der Datenbank.

### Fixed

- **Nur die wortgleiche Fassung wurde erkannt.** Steht eine Firma im
  Bestand mit einem Zusatz — etwa als „Werk XY (Konzern AB)", wie
  Namen aus Import-Quellen regelmäßig ankommen —, dann fand die Prüfung
  weder „Werk XY" noch „Konzern AB". **Und genau die kurze Form schreibt
  ein Mensch in einen Bericht.**

  Gefunden wurde das dadurch, dass zwei offene Meldungen einen echten
  Firmennamen enthielten und die Prüfung „sauber" meldete.

  Erkannt werden jetzt auch: der Name ohne Rechtsform, der Name ohne
  angehängten Zusatz hinter Bindestrich oder Komma, und der Teil vor
  einer Klammer.

- **Vermittler- und Portalnamen bleiben ausgenommen — auch mit Zusatz.**
  Sie sind ein Bestandteil des Produkts und stehen bewusst in Meldungen.
  Bisher griff die Ausnahme nur bei exakt gleicher Schreibweise: stand
  ein Vermittler im Bestand mit angehängter Rechtsform, war er **nicht**
  ausgenommen.

- **Einträge, die keine Namen sind, schweigen nicht mehr — sie werden
  als unsicher gemeldet.** Im Bestand stehen als „Firma" auch
  Technologiebegriffe und Platzhalter aus Importen. Sie tauchen jetzt
  mit einem Hinweis auf, statt entweder zu fehlen oder wie ein echter
  Treffer auszusehen.

### Hinweis

**Die Gegenrichtung war hier genauso wichtig wie die Erkennung
selbst.** Ein erster Entwurf hat auch den Inhalt von Klammern als
Suchbegriff genommen — gemessen am echten Bestand steht dort aber weit
öfter eine Anmerkung („Vermittler", „Beratung", ein Ortsname) als ein
zweiter Firmenname. Das hätte Fehlalarme über Dutzende Meldungen
erzeugt, und **eine Prüfung, der niemand mehr glaubt, schützt gar
nicht.** Der Klammerinhalt bleibt deshalb bewusst außen vor.

Für Nutzer ändert sich an der Bedienung nichts. Die Prüfung läuft weiter
automatisch vor jedem ausgehenden Text.
...
1. **ZIP herunterladen:** [PBP-1.7.53.zip](https://github.com/MadGapun/PBP/archive/refs/tags/v1.7.53.zip)
2. **Entpacken:** Rechtsklick auf die ZIP → *„Alle extrahieren..."* → Zielordner waehlen (z.B. `C:\PBP`). Darin liegt ein Unterordner `PBP-...` — dort hinein wechseln.
3. **Installieren:** Doppelklick auf **`INSTALLIEREN.bat`**
4. Das Setup laedt Python, alle Pakete und Chromium herunter (~3–5 Minuten) und konfiguriert Claude Desktop.
5. Auf dem Desktop liegt jetzt eine Verknuepfung **„PBP Bewerbungs-Portal"** — Doppelklick startet das Dashboard.
...
dein Profil einfach nichts hat — bisher sahen beide **exakt gleich aus**.
Und beide wurden nach fünf Läufen abgeschaltet.

### Fixed

- **Die Trefferzahl in der Quellen-Diagnose war bei zwölf Quellen schon
  gefiltert.** Was dort als „Rohtreffer" stand, war bei ihnen nicht, was
  die Börse geliefert hat, sondern was nach dem quelleneigenen
  Stichwort-Filter übrig blieb. Gemessen an einer Remote-Börse:

  | | |
  |---|---|
  | Was die Börse lieferte | 20 |
  | Was in der Diagnose stand | **0** |

- **Damit wurde die falsche Quelle abgeschaltet.** Genau das ist am
  1. September passiert: eine Börse galt als „liefert seit fünf Läufen
  nichts" und wurde stillgelegt — dabei lieferte sie zwanzig Stellen,
  und PBP konnte sie nur nicht mehr lesen (das war der Fehler aus
  v1.7.44).

- **Die Abschaltung entscheidet jetzt daran, ob die Quelle überhaupt
  etwas geliefert hat.** Wer liefert, ist nicht stumm — auch wenn nichts
  davon passt.

### Changed

- **Eine Quelle ohne fachliche Passung wird als solche benannt.** In der
  Quellen-Diagnose steht sie jetzt unter „liefert, aber nichts
  Passendes", zusammen mit der Zahl und dem geografischen Fokus. Das ist
  **kein Defekt**: die Börse arbeitet, sie ist nur die falsche für dein
  Profil. Ob sie eingeschaltet bleibt, entscheidest du — vorher wurde
  sie dir stillschweigend abgeschaltet.

### Hinweis

Die Zahlen füllen sich beim nächsten Suchlauf. Bis dahin steht bei den
betroffenen Quellen noch nichts — **und das heißt „noch nicht
gemeldet", nicht „null"**. Diese Unterscheidung ist der ganze Punkt
dieser Version.
...
1. **ZIP herunterladen:** [PBP-1.7.51.zip](https://github.com/MadGapun/PBP/archive/refs/tags/v1.7.51.zip)
2. **Entpacken:** Rechtsklick auf die ZIP → *„Alle extrahieren..."* → Zielordner waehlen (z.B. `C:\PBP`). Darin liegt ein Unterordner `PBP-...` — dort hinein wechseln.
3. **Installieren:** Doppelklick auf **`INSTALLIEREN.bat`**
4. Das Setup laedt Python, alle Pakete und Chromium herunter (~3–5 Minuten) und konfiguriert Claude Desktop.
5. Auf dem Desktop liegt jetzt eine Verknuepfung **„PBP Bewerbungs-Portal"** — Doppelklick startet das Dashboard.
...
`jobsuche_starten` nahm eine maximale Entfernung und einen
„nur Remote"-Schalter entgegen — **und las beides nie wieder**.

### Fixed

- **Zwei Parameter ohne Wirkung sind entfallen.** Wer bisher eine Suche
  mit „höchstens 15 km" startete, bekam Stellen aus dem ganzen Land.
  Kein Fehler, keine Meldung — die Werte wurden entgegengenommen,
  weitergereicht und nirgends gelesen. Der Hilfetext versprach dabei
  ausdrücklich eine Obergrenze.

  Sie sind ersatzlos weg, statt nachträglich verdrahtet zu werden. Als
  harte Filter würden sie zwei bewussten Entscheidungen widersprechen:
  **Entfernung ist ein Preis, kein Ausschluss** (eine weite Stelle
  rutscht nach unten, statt zu verschwinden), und ein
  „nur Remote"-Filter würde vor allem Anzeigen wegwerfen, die zum
  Arbeitsmodell **gar nichts sagen** — und das sind viele.

- **Beim Nachsehen kamen zwei weitere Fundstellen derselben Familie
  heraus.** „Maximale Entfernung" gab es an vier Stellen, und nur eine
  davon wurde tatsächlich gelesen:

  - In den Suchkriterien stand ein Einzelwert, gerechnet wurde gegen die
    Werte je Stellenart. **Im echten Bestand gemessen:** der Einzelwert
    stand auf 30 km, für Freelance galten 1500.
  - Die Anleitung für die Ersterfassung wies Claude an, den
    Entfernungswunsch unter einem Namen zu speichern, den es nicht gibt.
    Damit ging er ausgerechnet beim Einrichten verloren — dort, wo ihn
    fast jeder zum ersten Mal nennt.

### Added

- **Die Entfernung lässt sich jetzt mit einer Zahl setzen.** Ein Mensch
  sagt „höchstens 30 Kilometer", nicht eine Tabelle je Stellenart. Der
  Wert gilt für alle gewählten Stellenarten; wer unterschiedliche Werte
  braucht, setzt sie weiterhin einzeln — die genauere Angabe gewinnt,
  und PBP sagt, welche es genommen hat.

- **Ein wirkungsloser Altwert wird benannt statt still umgedeutet.**
  Steht in den Kriterien noch eine Entfernung, gegen die niemand
  rechnet, sagt PBP das beim Anzeigen und nennt beide Werte. Den Wert
  im Hintergrund neu zu deuten wäre eine Änderung an der Bewertung, die
  niemand veranlasst hat.

### Hinweis

Falls du bisher mit den beiden Schaltern gearbeitet hast: sie haben nie
etwas bewirkt, es ändert sich also nichts an deinen Ergebnissen — nur
daran, was PBP verspricht. Die Entfernung stellst du dauerhaft in den
Suchkriterien ein, „Remote" gehört in die Regionen-Liste.
...
1. **ZIP herunterladen:** [PBP-1.7.48.zip](https://github.com/MadGapun/PBP/archive/refs/tags/v1.7.48.zip)
2. **Entpacken:** Rechtsklick auf die ZIP → *„Alle extrahieren..."* → Zielordner waehlen (z.B. `C:\PBP`). Darin liegt ein Unterordner `PBP-...` — dort hinein wechseln.
3. **Installieren:** Doppelklick auf **`INSTALLIEREN.bat`**
4. Das Setup laedt Python, alle Pakete und Chromium herunter (~3–5 Minuten) und konfiguriert Claude Desktop.
5. Auf dem Desktop liegt jetzt eine Verknuepfung **„PBP Bewerbungs-Portal"** — Doppelklick startet das Dashboard.
...
-> {"status": "aktualisiert", "geaenderte_felder": ["title"]}
```

Geändert wurde nichts. Weder der Mensch noch Claude hatten eine
Möglichkeit, das zu bemerken.

### Fixed

- **Der Fehlschlag war bekannt — er wurde nur weggeworfen.** Die
  Datenbankebene meldet sauber zurück, ob wirklich ein Datensatz
  getroffen wurde. Die Ebene darüber hat diese Auskunft an **sieben
  Stellen** ignoriert und unbesehen „aktualisiert" geantwortet, samt
  Liste der angeblich geänderten Felder. Dasselbe beim Löschen.

  Ein stiller Fehlschlag mit Erfolgsmeldung ist von einem echten Erfolg
  nicht zu unterscheiden — danach arbeiten beide Seiten mit einem
  Profilstand weiter, den es nicht gibt.

- **Genau eine der acht Verzweigungen machte es richtig**, und der Weg
  über das Dashboard ebenfalls: dort kommt seit jeher eine ehrliche
  Fehlermeldung. Zwei Wege für dieselbe Frage, und der schwächere war
  der, den Claude nimmt. Die Auswertung liegt jetzt an **einer** Stelle;
  eine künftige achte Verzweigung erbt sie automatisch.

- **Die Absage sagt, was los ist, statt zu raten.** „Nichts
  geschrieben" hatte zwei mögliche Gründe — die Kennung gibt es nicht,
  oder es war kein änderbares Feld dabei. PBP sieht jetzt nach, statt
  sich für einen zu entscheiden: eine Antwort, die den Fragenden in die
  falsche Richtung schickt, ist schlimmer als gar keine. Sind **beide**
  falsch, wird die Kennung zuerst genannt — wer nur den Feldnamen
  korrigiert, schreibt sonst beim zweiten Versuch wieder ins Leere.

- **Kompetenzen verstehen jetzt auch deutsche Feldnamen.** Der Vorgänger
  hatte das für Berufserfahrung, Ausbildung und Projekte gebaut und den
  vierten Bereich übergangen. `niveau`, `kategorie` oder `berufsjahre`
  kommen an, statt still verworfen zu werden.

- **Nebenbefund, vom eigenen Test gefunden:** eine Kompetenz, deren Name
  als Extraktions-Artefakt abgewiesen wird, wurde trotzdem als
  „hinzugefügt" gemeldet — mit leerer Kennung. Derselbe stille
  Fehlschlag, nur beim Anlegen statt beim Ändern.

### Hinweis

Der Schutz gegen den Rückfall zählt die sieben Fundstellen nicht ab,
sondern **ruft jede Verzweigung auf** — vier Bereiche mal ändern und
löschen, jeweils mit einer Kennung, die es nicht gibt. Wer später eine
achte hinzufügt und die Auswertung vergisst, sieht das sofort.

An gespeicherten Daten ändert dieses Update nichts.
...
1. **ZIP herunterladen:** [PBP-1.7.46.zip](https://github.com/MadGapun/PBP/archive/refs/tags/v1.7.46.zip)
2. **Entpacken:** Rechtsklick auf die ZIP → *„Alle extrahieren..."* → Zielordner waehlen (z.B. `C:\PBP`). Darin liegt ein Unterordner `PBP-...` — dort hinein wechseln.
3. **Installieren:** Doppelklick auf **`INSTALLIEREN.bat`**
4. Das Setup laedt Python, alle Pakete und Chromium herunter (~3–5 Minuten) und konfiguriert Claude Desktop.
5. Auf dem Desktop liegt jetzt eine Verknuepfung **„PBP Bewerbungs-Portal"** — Doppelklick startet das Dashboard.
...
## [1.7.45] - 2026-09-08 — „Remote" heißt nicht „von überall"

Vom Nutzer bemerkt: *„Was habe ich mit US zu tun? Denke das ist ein
Fehler, zumal wir nur deutsche bzw. Quellen für den deutschsprachigen
Raum durchsuchen."*

Er hatte recht. In den Suchkriterien stehen Hamburg, Schleswig-Holstein,
Deutschland, DACH und 30 Kilometer Maximalentfernung — und trotzdem
konnten Stellen im Bestand landen, auf die man sich von hier aus gar
nicht bewerben kann.

### Fixed

- **Eine Stelle im Ausland ist kein Entfernungsproblem, sondern ein
  Ausschluss.** Die Ursache war eine einzige Regel: Sobald eine Stelle
  als „remote" markiert war, galt die Entfernung als **ohne Belang** —
  also **kein Abzug**, egal wo sie liegt. Für eine deutsche Remote-Stelle
  stimmt das. Für eine an die USA gebundene nicht: die ist von Hamburg
  aus nicht *weit weg*, sondern **nicht bewerbbar**. Arbeitserlaubnis,
  Arbeitsrecht, Kernzeit.

  „Remote" heißt nicht „von überall", sondern „ohne festen Bürositz
  **innerhalb eines Rechtsraums**".

- **Gemessen, bevor etwas geändert wurde.** Alle drei Remote-Börsen
  wurden live abgefragt: eine lieferte **20 von 20** Treffern außerhalb
  des deutschsprachigen Raums, die zweite 90 von 100, die dritte 3 von
  17. Zurück kamen Ortsangaben wie „Remote (United States)", „Remote
  (Romania)", „Remote (New Zealand)".

- **Kein Ausschluss auf Verdacht.** Aussortiert wird nur, wo ein Land
  **ausdrücklich dasteht**. Ein Ort wie „Bedford" oder schlicht „Remote"
  bleibt unbekannt und wird behandelt wie bisher — es gibt deutsche Orte
  mit fremd klingenden Namen, und eine zu Unrecht aussortierte Stelle ist
  teurer als eine zu hoch bewertete. Umgekehrt gilt: steht Deutschland,
  Österreich oder die Schweiz dabei — auch neben anderen Ländern —, ist
  die Stelle in Ordnung. „Europe" und „Worldwide" ebenso.

### Changed

- **Jede Quelle zeigt jetzt ihren geografischen Fokus.** Eine Börse mit
  globalem Zuschnitt ist nicht *kaputt*, wenn sie für eine Suche im
  deutschsprachigen Raum nichts bringt — sie ist die **falsche** Quelle.
  Ohne diese Angabe sah beides gleich aus.

- **Eine Quellenbeschreibung wurde korrigiert.** Sie versprach „gute
  DACH-Abdeckung" über einen Länderfilter der Schnittstelle. Der Filter
  wirkt nicht: 0 von 20 Treffern lagen im deutschsprachigen Raum. Diese
  Beschreibung ist vermutlich der Grund, warum die Quelle überhaupt
  eingeschaltet wurde.

### Hinweis

Die Prüfung greift in **beiden** Rechenwegen — in der Trefferliste und in
der Fit-Analyse. Eine Regel nur an einer von zwei Stellen einzubauen hat
dieses Projekt schon siebenmal Geld gekostet; ein Test vergleicht beide
Wege bei jedem Lauf.

Ob eine global ausgerichtete Quelle eingeschaltet bleibt, entscheidest
weiterhin du — PBP sagt jetzt nur, was sie liefert.
...
1. **ZIP herunterladen:** [PBP-1.7.45.zip](https://github.com/MadGapun/PBP/archive/refs/tags/v1.7.45.zip)
2. **Entpacken:** Rechtsklick auf die ZIP → *„Alle extrahieren..."* → Zielordner waehlen (z.B. `C:\PBP`). Darin liegt ein Unterordner `PBP-...` — dort hinein wechseln.
3. **Installieren:** Doppelklick auf **`INSTALLIEREN.bat`**
4. Das Setup laedt Python, alle Pakete und Chromium herunter (~3–5 Minuten) und konfiguriert Claude Desktop.
5. Auf dem Desktop liegt jetzt eine Verknuepfung **„PBP Bewerbungs-Portal"** — Doppelklick startet das Dashboard.
...
vollständig im eingeloggten Browser durchgespielt. Er funktioniert — mit
einer Ausnahme, die nur dort auffallen konnte.

### Fixed

- **LinkedIn entfernt eingefügtes HTML.** Der letzte Schritt des Ablaufs
  schreibt die gelesenen Anzeigen in die Seite, damit sie am Stück
  abgeholt werden können. Das geschah bisher als HTML — und LinkedIn
  filtert das heraus: Der Text stand danach zwar da, aber ohne jede
  Struktur, auch ohne die Trennlinien zwischen den Stellen. Angekommen
  wäre er, auseinandernehmen ließ er sich nicht mehr.

  Jetzt schreibt PBP reinen Text mit eindeutigen Trennmarken, die keine
  Filterung entfernen kann. Gegengemessen im laufenden Browser: 10.836
  Zeichen, alle Zeilenumbrüche erhalten, alle drei Stellen sauber
  getrennt.

### Bestätigt im selben Lauf

- Die Suchabfrage liefert **exakt dieselben Stellen wie die
  LinkedIn-Oberfläche** — gleiche Anzahl, gleiche Stellen, keine
  Abweichung in beide Richtungen.
- Zwei Suchbegriffe, eine Seite: 30 Treffer, alle mit Titel, Firma und
  Ort. Drei davon im Volltext geladen: 4.682, 1.461 und 4.118 Zeichen.
- Die Schnittstelle antwortet unverändert — die im August notierten
  Versionsnummern gelten weiter.

### Hinweis

Die vier Browser-Skripte waren gegen gespeicherte Antworten grün, und
ausgerechnet der eine Schritt, der die Seite selbst anfasst, war
trotzdem falsch. Ein Ablauf für einen fremden Browser gehört im fremden
Browser gemessen.
...
1. **ZIP herunterladen:** [PBP-1.7.43.zip](https://github.com/MadGapun/PBP/archive/refs/tags/v1.7.43.zip)
2. **Entpacken:** Rechtsklick auf die ZIP → *„Alle extrahieren..."* → Zielordner waehlen (z.B. `C:\PBP`). Darin liegt ein Unterordner `PBP-...` — dort hinein wechseln.
3. **Installieren:** Doppelklick auf **`INSTALLIEREN.bat`**
4. Das Setup laedt Python, alle Pakete und Chromium herunter (~3–5 Minuten) und konfiguriert Claude Desktop.
5. Auf dem Desktop liegt jetzt eine Verknuepfung **„PBP Bewerbungs-Portal"** — Doppelklick startet das Dashboard.
...
## [1.7.42] - 2026-09-07 — LinkedIn liefert wieder

LinkedIn stand in PBP auf „aktiv" und hatte seit dem 23. April keinen
einzigen Treffer geliefert. Erfolgsrate 0 %, die zweite Anbindung
abgeschaltet nach 24 Fehlern in Folge — ausgerechnet bei der Quelle, in
der die meisten Stellen tatsächlich ausgeschrieben werden.

Der Grund: LinkedIn blockt Abrufe von außen zuverlässig. Aus dem
**eingeloggten Browser** heraus läuft dieselbe Abfrage problemlos durch.
Genau dieser Weg wurde im August einmal komplett durchgespielt — 22
Suchbegriffe, 511 Treffer, 59 gelesene Anzeigen — und stand seitdem nur
in einer Notiz. Jetzt ist er ein Werkzeug.

### Added

- **`linkedin_lauf_plan()`** — sagt Claude, was zu tun ist: welche
  Suchbegriffe (aus deinem LinkedIn-Suchprofil, ergänzt um deine
  MUSS-Begriffe), welcher Zeitraum, und die fertigen Skripte für den
  Browser. Der Zeitraum ergibt sich aus deinem letzten Lauf, statt immer
  starr eine Woche zu nehmen — bei größerem Abstand ging bisher alles
  dazwischen verloren.

- **`linkedin_treffer_uebernehmen()`** — schreibt die Ernte nach PBP,
  über denselben Weg wie jede von Hand angelegte Stelle. Blacklist,
  Duplikat-Erkennung, Anker-Pflicht und Scoring gelten unverändert.

- **Der Trichter steht im Ergebnis:** wie viele Stellen roh gefunden
  wurden, wie viele nach dem Vorfilter blieben, wie viele Anzeigen
  gelesen wurden, wie viele angelegt — und mit welchem Grund der Rest
  übersprungen wurde. „0 angelegt" heißt damit nicht mehr dasselbe wie
  „0 gefunden".

### Changed

- **Ohne Anzeigentext wird nichts angelegt.** Von 59 Stellen, die nach
  dem Titel vielversprechend aussahen, blieben nach dem Lesen der
  vollständigen Anzeige **drei** übrig. Der nach Titel beste Treffer des
  ganzen Laufs verlangte im Fließtext ausgerechnet ein System, das auf
  deiner Ausschlussliste steht. Wer nur Titel und Kurztext übernimmt,
  bekommt genau die falschen Stellen mit hoher Punktzahl in die Liste.

- **LinkedIn gilt nicht mehr als veraltete Quelle.** Sie braucht einen
  eingeloggten Browser — das steht jetzt dran, statt dass sie still
  nichts liefert.

- **Kein Login heißt „wartet auf Login".** Das ist keine Aussage über
  den Stellenmarkt: die Quelle bleibt aktiv und wird nicht automatisch
  abgeschaltet.

### Hinweis

LinkedIn ändert gelegentlich interne Versionsnummern seiner Schnittstelle.
Passiert das, meldet PBP das als solches („Aufruf veraltet") statt
stillschweigend null Treffer zurückzugeben — der Unterschied zwischen
„nichts gefunden" und „gar nicht erst gefragt" ist der ganze Punkt.
...
1. **ZIP herunterladen:** [PBP-1.7.42.zip](https://github.com/MadGapun/PBP/archive/refs/tags/v1.7.42.zip)
2. **Entpacken:** Rechtsklick auf die ZIP → *„Alle extrahieren..."* → Zielordner waehlen (z.B. `C:\PBP`). Darin liegt ein Unterordner `PBP-...` — dort hinein wechseln.
3. **Installieren:** Doppelklick auf **`INSTALLIEREN.bat`**
4. Das Setup laedt Python, alle Pakete und Chromium herunter (~3–5 Minuten) und konfiguriert Claude Desktop.
5. Auf dem Desktop liegt jetzt eine Verknuepfung **„PBP Bewerbungs-Portal"** — Doppelklick startet das Dashboard.
...
- **Der Analyseplan schlug 962 Dokument-Zuordnungen vor (#991).** Wer
  ihn aufrief — und die Anleitung empfiehlt ihn als **ersten** Schritt —
  bekam eine Liste, mit der sich nicht arbeiten liess: einer einzigen,
  längst abgeschlossenen Bewerbung wurden rund dreissig Dokumente
  angeboten, darunter Lebensläufe, die erkennbar für acht ganz andere
  Firmen geschrieben worden waren. Dieselben Dokumente tauchten
  zusätzlich bei weiteren Bewerbungen auf.

  Die Ursache: der Plan suchte den Firmennamen im **Volltext** jedes
  Dokuments. Ein Firmenname im Fliesstext ist aber kein Hinweis auf
  Zugehörigkeit — er steht in jeder Absage, jeder Signatur und in jedem
  Anschreiben, das die Firma nur adressiert.

  Dieselbe Frage wurde an anderer Stelle längst besser beantwortet:
  `dokumente_ohne_bewerbung` lieferte für denselben Bestand **8 Fälle**,
  jeden mit Begründung, Konfidenz und Beleg. Beide Wege sind jetzt
  derselbe. Der Plan liefert nur noch Vorschläge mit benanntem
  Verdachtsmoment, nennt zu jedem die Konfidenz und den Beleg, macht
  **keinen** Vorschlag, wo kein Ziel erkennbar ist, und ist auf 15
  Einträge gedeckelt — die vollständige Liste holt weiterhin
  `dokumente_ohne_bewerbung`.

  Der Volltext bleibt ein Signal, aber ein enges: nur bei Korrespondenz
  (Mails, Absagen, Zusagen, Recruiter-Anfragen) und nur, wenn genau
  **eine** Firma aus deinem Bestand darin vorkommt. Damit funktioniert
  der ursprüngliche Fall weiter, für den das Ganze einmal gebaut wurde:
  eine Mail, deren Dateiname nichts sagt, deren Text aber die Firma
  nennt.

- **Die Warnung vor abgeschlossenen Bewerbungen gilt jetzt überall.**
  „Diese Bewerbung ist schon abgeschlossen — nur verknüpfen, wenn das
  Dokument eindeutig dazugehört" stand nur im Analyseplan, obwohl beide
  Wege dieselben Vorschläge machen. Eine Warnung an einem von zwei Wegen
  ist keine Warnung.

### Changed

- **Store-Installation von Claude Desktop: die App-Kennung kommt jetzt
  aus dem Paket** statt fest aus dem Code — ein Vorschlag aus der
  Diskussion zu #990. Vorher hätte eine Umbenennung durch Anthropic dazu
  geführt, dass Claude nach der Installation still nicht mehr
  automatisch startet.
---
...
1. **ZIP herunterladen:** [PBP-1.7.38.zip](https://github.com/MadGapun/PBP/archive/refs/tags/v1.7.38.zip)
2. **Entpacken:** Rechtsklick auf die ZIP → *„Alle extrahieren..."* → Zielordner waehlen (z.B. `C:\PBP`). Darin liegt ein Unterordner `PBP-...` — dort hinein wechseln.
3. **Installieren:** Doppelklick auf **`INSTALLIEREN.bat`**
4. Das Setup laedt Python, alle Pakete und Chromium herunter (~3–5 Minuten) und konfiguriert Claude Desktop.
5. Auf dem Desktop liegt jetzt eine Verknuepfung **„PBP Bewerbungs-Portal"** — Doppelklick startet das Dashboard.
...
aufgefallen, die beim Lesen des Codes unsichtbar waren und im Browser
kaum stoerten — auf dem Bild aber sofort.

### Fixed

- **Der obere Block lief rechts aus dem Fenster.** Die Karten „Offen",
  „Naechster sinnvoller Schritt" und der Tagesimpuls waren breiter als
  der Inhaltsbereich; am rechten Rand fehlten Beschriftungen. Der Fehler
  ist aelter als der neue Block — sichtbar wurde er erst, als dort eine
  Liste mit Datum und Herkunft stand. Im Block „Offen" selbst waren aus
  demselben Grund die Herkunft-Etiketten abgeschnitten.

- **Die Vorbereitungszeile stand auf demselben Tag wie das Gespraech.**
  Damit standen „Vorbereiten: Zweitgespraech" und „Zweitgespraech"
  direkt untereinander — dieselbe Doppelung, gegen die das Dashboard
  gerade aufgeraeumt wurde. Die Vorbereitung ist jetzt am Vortag
  faellig. Ist das Gespraech heute oder morgen, bleibt sie bei heute:
  sonst waere sie im selben Moment „ueberfaellig", ohne dass du etwas
  versaeumt haettest.

### Changed

- **Die Screenshots in der Dokumentation sind neu.** Sie zeigen den
  Dashboard-Stand ab v1.7.31, weiterhin nur mit Musterdaten.
...
1. **ZIP herunterladen:** [PBP-1.7.34.zip](https://github.com/MadGapun/PBP/archive/refs/tags/v1.7.34.zip)
2. **Entpacken:** Rechtsklick auf die ZIP → *„Alle extrahieren..."* → Zielordner waehlen (z.B. `C:\PBP`). Darin liegt ein Unterordner `PBP-...` — dort hinein wechseln.
3. **Installieren:** Doppelklick auf **`INSTALLIEREN.bat`**
4. Das Setup laedt Python, alle Pakete und Chromium herunter (~3–5 Minuten) und konfiguriert Claude Desktop.
5. Auf dem Desktop liegt jetzt eine Verknuepfung **„PBP Bewerbungs-Portal"** — Doppelklick startet das Dashboard.
...
deines Berufs aus dem, was es ohnehin sammelt.

### Fixed

- **#971 — die Kompetenz-Erkennung kannte nur ein Berufsfeld.** Seit
  v1.7.24 arbeitet sie mit einer Wortliste. Die war fuer Technik
  richtig zusammengestellt und fuer alles andere leer: bei Pflege,
  Erziehung und Grafik erkannte sie **null** Fachbegriffe, und die
  Passungsquote in der Skill-Analyse hatte damit keine Grundlage.

  Statt die Liste zu verlaengern — sie waere immer nur so gut wie ihre
  letzte Pflege — lernt PBP jetzt aus zwei Quellen, die an kein
  Berufsfeld gebunden sind:

  * **Dein Profil.** Was du selbst aufgeschrieben hast, ist die
    verlaesslichste Auskunft ueber dein Fach.
  * **Die gesammelten Anzeigen.** Ein Begriff, der in mehreren
    Ausschreibungen vorkommt, aber nicht in fast allen, ist ein
    Fachbegriff. Was ueberall steht, ist Floskel.

  An acht Pflege-Anzeigen gemessen lernt PBP so Beatmung,
  Vitalparameter, Behandlungspflege und Pflegeplanung. Eine neue
  Anzeige liefert damit vier Fachbegriffe statt keinem.

  Die bisherige Liste bleibt als Startwert erhalten — sie ist fuer ihr
  Feld richtig. Sie ist nur nicht mehr die Grenze.

### Changed

- Tests: 2703 passed, 2 skipped. MCP-Tools 206, Schema v48
  unveraendert.
...
1. **ZIP herunterladen:** [PBP-1.7.30.zip](https://github.com/MadGapun/PBP/archive/refs/tags/v1.7.30.zip)
2. **Entpacken:** Rechtsklick auf die ZIP → *„Alle extrahieren..."* → Zielordner waehlen (z.B. `C:\PBP`). Darin liegt ein Unterordner `PBP-...` — dort hinein wechseln.
3. **Installieren:** Doppelklick auf **`INSTALLIEREN.bat`**
4. Das Setup laedt Python, alle Pakete und Chromium herunter (~3–5 Minuten) und konfiguriert Claude Desktop.
5. Auf dem Desktop liegt jetzt eine Verknuepfung **„PBP Bewerbungs-Portal"** — Doppelklick startet das Dashboard.
...
Dritter Schritt aus der Berufsfeld-Recherche: PBP kennt jetzt den
ganzen Arbeitsmarkt, nicht nur den technischen Teil.

### Fixed

- **#970 — vier von sechs Berufen wurden falsch oder gar nicht
  eingeordnet.** Aus dem Profil leitet PBP ab, welche Jobboersen es
  empfiehlt. Eine Erzieherin galt dabei als Tech-Einsteigerin, ein
  Elektroniker mit fuenfzehn Jahren Erfahrung als "nicht einzuordnen".

  Der lehrreichste Einzelfall: **"Kita" enthaelt "ki"**. Das Kuerzel
  fuer kuenstliche Intelligenz wurde als Teil eines Wortes gefunden, und
  damit bekam eine Erzieherin neun internationale Tech-Boards
  empfohlen. Kurze Kuerzel werden jetzt als ganzes Wort gesucht.

  Sechs Berufsfelder sind dazugekommen, die vorher komplett fehlten:
  Gesundheit und Pflege, Erziehung und Bildung, Handel und Logistik,
  Gastronomie, Kreativ und Medien, Verwaltung und Finanzen. Das
  Handwerk kennt jetzt auch die heute gebraeuchlichen
  Ausbildungsberufe — bisher stand dort der "Elektriker", aber nicht der
  "Elektroniker".

- **Wenn PBP das Berufsfeld nicht erkennt, sagt es das jetzt.** Und es
  empfiehlt dann BREIT statt schmal: vorher bekam ausgerechnet ein nicht
  eingeordnetes Profil die kuerzeste Quellenliste, ohne zu erfahren,
  dass diese Auswahl aus Ratlosigkeit entstand und nicht aus Passung.

### Changed

- Geprueft an elf Lebenslaeufen quer durch den Arbeitsmarkt, darunter
  vier technische zur Gegenprobe. Alle elf werden richtig zugeordnet.
- Tests: 2689 passed, 2 skipped. MCP-Tools 206, Schema v48
  unveraendert.
...
1. **ZIP herunterladen:** [PBP-1.7.29.zip](https://github.com/MadGapun/PBP/archive/refs/tags/v1.7.29.zip)
2. **Entpacken:** Rechtsklick auf die ZIP → *„Alle extrahieren..."* → Zielordner waehlen (z.B. `C:\PBP`). Darin liegt ein Unterordner `PBP-...` — dort hinein wechseln.
3. **Installieren:** Doppelklick auf **`INSTALLIEREN.bat`**
4. Das Setup laedt Python, alle Pakete und Chromium herunter (~3–5 Minuten) und konfiguriert Claude Desktop.
5. Auf dem Desktop liegt jetzt eine Verknuepfung **„PBP Bewerbungs-Portal"** — Doppelklick startet das Dashboard.
...
Zweiter Schritt aus der Berufsfeld-Recherche: PBP erkennt jetzt, dass
derselbe Beruf verschiedene Namen hat.

### Fixed

- **#969 — derselbe Beruf unter anderem Namen fiel aus der Liste.** Wer
  "Pflegefachkraft" als Pflichtbegriff eintraegt, sah
  "Gesundheits- und Krankenpflegerin" nicht — obwohl das derselbe Beruf
  ist, die Amtsbezeichnung vor der Pflegeberufereform 2020, die heute
  noch in vielen Ausschreibungen steht. Alternativbezeichnungen kannte
  PBP nur fuer vier technische Produktnamen.

  Jetzt fragt PBP beim Suchlauf einmal nach, wie derselbe Beruf sonst
  noch heisst, und findet damit auch Anzeigen, die eine andere
  Bezeichnung verwenden. Fuer "Pflegefachkraft" sind das Altenpfleger,
  Pflegefachmann und -frau, Gesundheits- und Krankenpfleger,
  Krankenschwester.

  Das funktioniert in jedem Berufsfeld, weil die Bezeichnungen aus dem
  Stellenmarkt selbst kommen und nicht aus einer Liste im Programm —
  eine Liste waere immer nur so gut wie ihre letzte Pflege.

  Fachfremde Anzeigen bleiben draussen: eine Alternativbezeichnung
  erweitert die Suche, sie weicht sie nicht auf.

- **Nachtrag zu #949 aus v1.7.26.** Das Anzeigenalter wurde bei der
  Bundesagentur nie erkannt — der Feldname im Programm war geraten und
  falsch geschrieben. Beim Nachschlagen der echten Antwort aufgefallen
  und behoben; die Laufzeit einer Anzeige wird jetzt tatsaechlich
  angezeigt.

### Changed

- Tests: 2664 passed, 2 skipped. MCP-Tools 206, Schema v48
  unveraendert.
...
1. **ZIP herunterladen:** [PBP-1.7.28.zip](https://github.com/MadGapun/PBP/archive/refs/tags/v1.7.28.zip)
2. **Entpacken:** Rechtsklick auf die ZIP → *„Alle extrahieren..."* → Zielordner waehlen (z.B. `C:\PBP`). Darin liegt ein Unterordner `PBP-...` — dort hinein wechseln.
3. **Installieren:** Doppelklick auf **`INSTALLIEREN.bat`**
4. Das Setup laedt Python, alle Pakete und Chromium herunter (~3–5 Minuten) und konfiguriert Claude Desktop.
5. Auf dem Desktop liegt jetzt eine Verknuepfung **„PBP Bewerbungs-Portal"** — Doppelklick startet das Dashboard.
...
> denen PBP etwas behauptet hat, das die Daten nicht hergaben. Keine
> Schema-Migration.

### Fixed
- **Der Anzeigentext wird vollstaendig gespeichert (#952)**: Bisher
  wurde er bei exakt 2000 Zeichen abgeschnitten — in **jedem** der 26
  Quellen-Adapter. Getroffen hat das systematisch den wichtigsten Teil:
  Stellenanzeigen beginnen mit der Unternehmensvorstellung, die
  Anforderungen stehen am Ende. Abschluss, Berufsjahre, Sprachen und
  Befristungshinweise fehlten damit reihenweise.

  Besonders unangenehm: auch das **Nachladen** kappte, sodass wiederholtes
  Nachladen zuverlaessig denselben halben Text holte. Und der Fehler war
  stumm — nichts an der Ausgabe deutete darauf hin.

  Wer Bestandsdaten heilen will: `beschreibungen_nachladen_bestand()`
  zeigt zuerst nur an, wie viele betroffen sind.

- **Nachfassungen sagen, was zu tun ist (#945)**: Fuenf von sieben
  offenen Nachfassungen hatten ein leeres Beschreibungsfeld — wer sie
  oeffnete, sah Firma und Datum und musste den Rest selbst
  zusammensuchen. Jetzt steht dort, an wen, ueber welchen Kanal, worauf
  bezogen. Dazu ein fertiger Claude-Auftrag per Knopf.

  Nachfassungen, die der Verfahrensstand ueberholt hat (Gespraech laeuft
  bereits, Termin steht), werden automatisch hinfaellig statt weiter
  angemahnt. Und die Sortierung richtet sich nach dem Verfahrensstand:
  die Nachfrage zu einem abgeschlossenen Gespraech steht jetzt vor einer
  Routine-Nachfrage, auch wenn deren Datum aelter ist.

- **Die Statistik klassifiziert nach Inhalt (#943)**: Die Kategorie
  "automatische Ablehnung" war zu vier Fuenfteln falsch befuellt. Sie
  entstand allein aus dem Zeitabstand — und bei nachtraeglich erfassten
  Altbewerbungen stehen Bewerbungs- und Absagedatum am selben Tag.
  13 Faelle trugen woertlich "Keine Rueckmeldung", also das Gegenteil.

  Das ist mehr als ein Schoenheitsfehler: die beiden Kategorien fuehren
  zu **entgegengesetzten** Handlungen — Unterlagen pruefen gegenueber
  frueher nachfassen.

  Ausserdem: Ablehnung und Versanden werden getrennt ausgewiesen (ein
  abgelaufener Vorgang ist keine Absage), Zeitkennzahlen lassen den
  Null-Tage-Artefakt aussen vor, und Vermutungen ("vermutlich...") sind
  als solche gekennzeichnet statt als Befund gefuehrt.

- **Der Bewerbungsbericht rechnet richtig (#944)**: Er nannte drei
  verschiedene Ablehnungszahlen, und die Aufwandstabelle addierte
  sichtbar falsch. Fuer ein Dokument, das bei der Arbeitsagentur
  vorgelegt wird, ist das teuer.

  Dazu lesbar gemacht: die Bewerbungsliste bricht keine Firmennamen mehr
  mitten im Wort ab, das Aktivitaetsprotokoll ist vollstaendig statt
  abgeschnitten, das Sternchen in der Score-Spalte hat eine Legende, und
  die Sichtungsleistung steht auf Seite 1 statt auf Seite 5.

- **Elwosa wiederholt sich nicht mehr (#924)**: Dieselbe Zeile stand
  dreimal wortgleich untereinander. Die Sperre sitzt jetzt dort, wo
  jede Nachricht durchmuss.

- **Der Quellen-Check prueft, ob Stellen kommen (#808)**: Bisher zaehlte
  nur, ob ein Server antwortet. Eine Quelle, die mit HTTP 200 eine leere
  Liste zurueckgibt, galt als gesund — Erfolgsraten von 93 bis 98 Prozent
  fuer Quellen, die seit Wochen nichts lieferten.

- **Abschluss-Anforderungen werden zuverlaessiger erkannt (#955)**:
  Beschreibende Formulierungen ("Dein Studium bildet die Ausgangsbasis")
  galten als "kein Abschluss gefordert". Werkstudenten- und
  Praktikumsanzeigen loesen dabei bewusst keine Anforderung aus — dort
  ist das Studium die Zielgruppe.
...
1. **ZIP herunterladen:** [PBP-1.7.23.zip](https://github.com/MadGapun/PBP/archive/refs/tags/v1.7.23.zip)
2. **Entpacken:** Rechtsklick auf die ZIP → *„Alle extrahieren..."* → Zielordner waehlen (z.B. `C:\PBP`). Darin liegt ein Unterordner `PBP-...` — dort hinein wechseln.
3. **Installieren:** Doppelklick auf **`INSTALLIEREN.bat`**
4. Das Setup laedt Python, alle Pakete und Chromium herunter (~3–5 Minuten) und konfiguriert Claude Desktop.
5. Auf dem Desktop liegt jetzt eine Verknuepfung **„PBP Bewerbungs-Portal"** — Doppelklick startet das Dashboard.
...
> Quelle wurde live geprueft und entweder repariert oder sichtbar als
...
- **Dritte wiederhergestellte Quelle**: Eine Ingenieur-Jobboerse
  ...
  Stellen-Detailseiten. Jetzt **24 Stellen mit Link**, fachlich im
  Zielbereich (Konstruktion, Anlagenbau, Entwicklung).
  Der vorhandene Guard-Test prueft nur, ob die neue Domain im Code
...
- **Tote Quellen werden ausgegraut statt still mitgeschleppt**:
  ...
  mehr, eine Seite antwortet mit 404, zwei blocken automatische Zugriffe
  (403). Wo es einen Weg ueber den Browser gibt, steht er als Hinweis
  dabei; fuer die betroffene Projektboerse deckt die in v1.7.19
  reparierte Alternative dieselben Projekttypen ab.
- **Keine irrefuehrenden Erreichbarkeits-Proben mehr**: Defekte Quellen
  werden nicht mehr angepingt — eine Probe meldet dort HTTP 200 und
  damit faelschlich „gruen", obwohl keine Stelle geliefert wird.
...
1. **ZIP herunterladen:** [PBP-1.7.20.zip](https://github.com/MadGapun/PBP/archive/refs/tags/v1.7.20.zip)
2. **Entpacken:** Rechtsklick auf die ZIP → *„Alle extrahieren..."* → Zielordner waehlen (z.B. `C:\PBP`). Darin liegt ein Unterordner `PBP-...` — dort hinein wechseln.
3. **Installieren:** Doppelklick auf **`INSTALLIEREN.bat`**
4. Das Setup laedt Python, alle Pakete und Chromium herunter (~3–5 Minuten) und konfiguriert Claude Desktop.
5. Auf dem Desktop liegt jetzt eine Verknuepfung **„PBP Bewerbungs-Portal"** — Doppelklick startet das Dashboard.
...
> (Schema **v48** unveraendert), keine neuen Abhaengigkeiten.

### Fixed
- **Status-Dropdown ohne tote Werte** (#896): `Entwurf` und
  `Warte auf Rueckmeldung` entfernt — das Backend lehnt ersteres ab und
  mappte letzteres still auf `Eingangsbestaetigung`; die Auswahl wurde
  also nie so gespeichert, wie sie angezeigt war. Bestehende alte
  Datensaetze werden weiterhin korrekt dargestellt.
- **Einstellungen-Tab „Bewertung" per Sidebar erreichbar** (#896): der
  Tab (#663, Ablehnungsgruende-Editor) existierte, fehlte aber in der
  Sidebar-Subnavigation.
- **Testhaerte Smart-Default-Quellen** (#857): der Warte-Helfer der
  #744-Suite joint jetzt die pbp-Worker-Threads, statt nur auf den
  Job-Status zu warten — der Such-Thread laeuft nach `fertig` noch nach
  (Auto-Aussortierungs-Hook) und teilte sich die SQLite-Connection mit
  den Test-Asserts; das machte den CI-Volllauf sporadisch rot.

### Added
- **Paritaets-Guards `tests/test_frontend_paritaet_896.py`**:
  STATUS_OPTIONS gegen die Backend-Whitelist und Sidebar-Subnavigation
  gegen die Einstellungs-Tabs, beides aus den Quelldateien geparst —
  dieselbe Absicherungsklasse wie der PAGE_IDS-Guard aus v1.7.13.
...
1. **ZIP herunterladen:** [PBP-1.7.14.zip](https://github.com/MadGapun/PBP/archive/refs/tags/v1.7.14.zip)
2. **Entpacken:** Rechtsklick auf die ZIP → *„Alle extrahieren..."* → Zielordner waehlen (z.B. `C:\PBP`). Darin liegt ein Unterordner `PBP-...` — dort hinein wechseln.
3. **Installieren:** Doppelklick auf **`INSTALLIEREN.bat`**
4. Das Setup laedt Python, alle Pakete und Chromium herunter (~3–5 Minuten) und konfiguriert Claude Desktop.
5. Auf dem Desktop liegt jetzt eine Verknuepfung **„PBP Bewerbungs-Portal"** — Doppelklick startet das Dashboard.
...
## [1.7.13] - 2026-08-12 — Hotfix: Aufgaben-Tab Deep-Link/2-Klick-Bug (#846) + Elwosa-unbegrenzt-Vertrag (#853)

> **Update fuer alle v1.7-Nutzer, die den Aufgaben-Bereich nutzen.**
> KEINE Schema-Migration (Schema **v48** unveraendert), keine neuen
> Abhaengigkeiten.

### Fixed
- **Elwosa: `frequency='unbegrenzt'` hebt jetzt wirklich die Kontingente
  auf** (#853): Das Ambiente-Tageskontingent stand VOR dem
  unbegrenzt-Shortcut und drosselte Power-User entgegen dem
  dokumentierten #822-Vertrag („Kontingente entfallen, Sperrfristen
  bleiben"). Kind-Sperrfrist, Ruhezeit, Rueckschlag-Sperre und
  Ungelesen-Daempfung gelten unveraendert. Nebeneffekt behoben: der
  CI-Lauf war dadurch tageszeitabhaengig rot. Zusaetzlich zieht
  `_pick_valid_line` ohne Wiederholung (eine kaputte Linie im Pool
  konnte Elwosa per Zufallspech verstummen lassen) und fuenf
  Alt-Test-Fixtures sind Ruhezeit-fest.
- **Aufgaben-Tab: Deep-Link und erster Klick funktionieren** (#846):
  `aufgaben` fehlte in `PAGE_IDS` (`frontend/src/utils.js`) — exakt
  derselbe Fehler wie bei `kontakte` in v1.7.0-beta.21. Folgen bisher:
  ein Lesezeichen oder Reload auf `#aufgaben` landete auf dem Dashboard,
  und der erste Klick auf „Aufgaben" in der Sidebar (oder „Zu den
  Aufgaben" im Ueberfaellig-Banner) prallte ab — erst der zweite blieb
  stabil. Gefunden bei der Screenshot-Erzeugung der Sichtbarkeits-Welle.

### Added
- **Guard-Test `tests/test_frontend_page_ids_846.py`**: liest
  `TAB_CONFIG` (App.jsx) und `PAGE_IDS` (utils.js) und schlaegt fehl,
  sobald ein Tab ohne `PAGE_IDS`-Eintrag dazukommt — der Fehler kann
  beim naechsten neuen Tab nicht mehr passieren.
...
1. **ZIP herunterladen:** [PBP-1.7.13.zip](https://github.com/MadGapun/PBP/archive/refs/tags/v1.7.13.zip)
2. **Entpacken:** Rechtsklick auf die ZIP → *„Alle extrahieren..."* → Zielordner waehlen (z.B. `C:\PBP`). Darin liegt ein Unterordner `PBP-...` — dort hinein wechseln.
3. **Installieren:** Doppelklick auf **`INSTALLIEREN.bat`**
4. Das Setup laedt Python, alle Pakete und Chromium herunter (~3–5 Minuten) und konfiguriert Claude Desktop.
5. Auf dem Desktop liegt jetzt eine Verknuepfung **„PBP Bewerbungs-Portal"** — Doppelklick startet das Dashboard.
...
> (Schema **v48** unveraendert). Nach dem Update Claude Desktop komplett
...
- **`termin_dubletten_bereinigen()`** (jetzt **193** MCP-Tools): findet
...
`lerninsights.py` neu geschrieben. 39 neue Tests, Suite: **2082 passed,
1 skipped**. Zwei Folge-Issues aus dieser Runde: #808 (Health-Check meldet
...
1. **ZIP herunterladen:** [PBP-1.7.11.zip](https://github.com/MadGapun/PBP/archive/refs/tags/v1.7.11.zip)
2. **Entpacken:** Rechtsklick auf die ZIP -> *„Alle extrahieren..."* -> Zielordner waehlen (z.B. `C:\PBP`). Darin liegt ein Unterordner `PBP-...` — dort hinein wechseln.
3. **Installieren:** Doppelklick auf **`INSTALLIEREN.bat`**
4. Das Setup laedt Python, alle Pakete und Chromium herunter (~3-5 Minuten) und konfiguriert Claude Desktop.
5. Auf dem Desktop liegt jetzt eine Verknuepfung **„PBP Bewerbungs-Portal"** — Doppelklick startet das Dashboard.
...
> **Empfohlenes Update fuer alle v1.7-Nutzer.** KEINE Schema-Migration
> (Schema **v48** unveraendert — die neue `learned_insights`-Tabelle kommt
...
1. **ZIP herunterladen:** [PBP-1.7.10.zip](https://github.com/MadGapun/PBP/archive/refs/tags/v1.7.10.zip)
2. **Entpacken:** Rechtsklick auf die ZIP -> *„Alle extrahieren..."* -> Zielordner waehlen (z.B. `C:\PBP`). Darin liegt ein Unterordner `PBP-...` — dort hinein wechseln.
3. **Installieren:** Doppelklick auf **`INSTALLIEREN.bat`**
4. Das Setup laedt Python, alle Pakete und Chromium herunter (~3-5 Minuten) und konfiguriert Claude Desktop.
5. Auf dem Desktop liegt jetzt eine Verknuepfung **„PBP Bewerbungs-Portal"** — Doppelklick startet das Dashboard.
