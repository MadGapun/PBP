  ...
  return (
    <Card className="rounded-2xl">
      ...
      <div className="grid gap-3">
        {status?.konfiguriert ? (
          <p className="text-sm text-teal">
            Verbunden — Adzuna läuft bei aktivierter Quelle im Suchlauf mit.
...
function ErweiterungenTab({ pushToast }) {
  ...
  async function pairPlugin() {
    ...
    try {
      ...
      pushToast("Das Manifest ist kein gültiges JSON.", "danger");
      return;
    }
    setPluginBusy(true);
    try {
  ...
  async function savePath(name) {
    ...
    try {
      ...
      pushToast(`Pfad nicht übernommen: ${error.message}`, "danger");
    } finally {
      setBusy("");
    }
  }
    ...
    <>
      ...
      <Card className="rounded-2xl">
        <SectionHeading
          ...
          description="Zusatzprogramme, die PBP-Funktionen freischalten. Nichts wird ohne deine Zustimmung heruntergeladen; Größe und Lizenz stehen immer dabei."
        />
                ...
                <div className="mt-3 flex flex-wrap items-center gap-2">
                  {!k.verfuegbar && !laeuft ? (
                    <Button
                      ...
                      title={`Lädt ~${k.groesse_mb} MB herunter und installiert nach AppData (kein Admin nötig).`}
                    >
      ...
      <Card className="rounded-2xl">
        <SectionHeading
          ...
          description="Externe Programme (z.B. Watch-Folder, Mail-Add-ons), die über die lokale Ingest-API Stellen oder E-Mails an PBP liefern. Alles bleibt auf diesem Rechner (127.0.0.1)."
        />
        {pairResult ? (
        ...
        {plugins.length === 0 && !pairResult ? (
          <p className="text-sm text-muted">
            Noch kein Plugin gekoppelt. Referenz-Beispiel: <code>plugins/watch-folder</code> im PBP-Repo — Mails per Ordner an PBP übergeben.
          </p>
        ) : null}
        <div className="grid gap-3">
          {plugins.map((p) => (
        ...
        <div className="mt-4">
          {pairOpen ? (
            <div className="rounded-xl border border-line/40 bg-shell/40 p-4">
              <p className="text-sm text-muted">
                Manifest des Plugins einfügen (Datei <code>pbp-plugin.json</code>, liegt dem Plugin bei):
              </p>
              <textarea
                value={manifestText}
