...
def run():
    uvicorn.run(app, host="127.0.0.1", port=8200, log_level="error")
...
try:
    resp = urllib.request.urlopen("http://127.0.0.1:8200/api/status", timeout=5)
    data = resp.read().decode()
    print(f"  Dashboard erreichbar auf http://localhost:8200")
...
if ($dashOK) {
    Write-OK "Dashboard funktioniert auf http://localhost:8200"
} else {
