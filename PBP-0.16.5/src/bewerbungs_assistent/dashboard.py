...
Serves a browser-based UI on localhost:5173 for visual management.
...
DASHBOARD_PORT = int(os.environ.get("BA_DASHBOARD_PORT", "5173"))

def start_dashboard(db_instance, port: int = None):
    """Start the dashboard web server (called from background thread).

    Port priority: port argument > BA_DASHBOARD_PORT env var > 5173
    """
    global _db
    _db = db_instance
