...
def register(mcp, db, logger):
    ...
    def dokumente_text_nachziehen(dokument_id: str = "", anwenden: bool = False,
        ...
        **Es wird nur überschrieben, wenn dabei MEHR herauskommt.** Ein
        ...
        """
        from pathlib import Path
        from ..dashboard import _extract_document_text
        ...
        for doc in kandidaten:
            ...
            geprueft += 1
            try:
                neu, _kontext, befund = _extract_document_text(pfad)
            except Exception as exc:
                uebersprungen.append({"dateiname": doc.get("filename", ""),
    ...
    def newsletter_quelle_markieren(dokument_id: str) -> dict:
        ...
        PBP lernt Absender-Domain (+ Betreff-Anfang) — künftige Mails
        ...
        Für die grossen Portale (StepStone, LinkedIn, XING, Indeed,
        Arbeitsagentur, freelance.de, JobLeads) ist das NICHT nötig —
        die erkennt PBP von selbst. Dieses Tool ist für alles andere
        (Firmen-Newsletter, Nischen-Börsen).
        ...
        return {
            ...
            "verarbeitung": result,
            "hinweis": (
                "Künftige Mails dieser Quelle werden beim Upload/Ingest "
    ...
    def newsletter_verarbeiten(dokument_id: str) -> dict:
        ...
        erscheinen zunächst als 'unbewertet' — der Auto-Refetch lädt
        die Beschreibungen nach (#622/#756/C23 greifen ineinander).

        Beim Upload passiert das für ERKANNTE Newsletter automatisch —
        dieses Tool ist für Altbestand oder nicht erkannte Formate
        ...
        if not quelle and result.get("status") == "uebernommen":
            result["hinweis_quelle"] = (
                ...
                "sie für künftige Mails."
            )
        return result

    @mcp.tool()
    def dokument_ocr_ausfuehren(dokument_id: str, max_seiten: int = 15) -> dict:
        """Führt OCR für ein gescanntes PDF aus (E19/#750, v1.8.0-beta.0).

        Nutzt die Tesseract-Komponente (I10) — KI-frei, läuft komplett
        lokal. Der erkannte Text wird mit Provenienz-Header gespeichert
        (wie dokument_text_setzen). Typischer Einsatz: ein früher
        hochgeladenes Zeugnis ohne Text-Ebene nachziehen, nachdem die
        Komponente installiert wurde.

        Ist die Komponente nicht installiert, kommt ein ANGEBOT zurück —
        ...
        if not filepath.lower().endswith(".pdf"):
            return {"fehler": "OCR gibt es nur für PDF-Dokumente."}
        from pathlib import Path
        if not Path(filepath).is_file():
        ...
        antwort = {
            ...
            "provenienz": header,
            "hinweis": (
                "Nächste Schritte: dokument_profil_extrahieren() falls der "
                "Inhalt ins Profil soll, dokument_status_setzen() für den "
                "Extraktions-Status."
            ),
        }
